...
export default function DODocuments({ onNavigate = () => { }, currentPage = 'documents' }: any) {
  ...
  return (
    <Layout darkMode={darkMode} setDarkMode={setDarkMode} onNavigate={onNavigate} currentPage={currentPage} topBarCaption="🚚 ใบส่งสินค้า">
      <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center gap-2 mb-6 text-sm">
            ...
            <span className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>🚚 ใบส่งสินค้า</span>
          </div>

          {/* Header */}
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
              <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{cfg.icon} {cfg.labelTh}</h1>
              <p className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                จัดการใบส่งสินค้า — ตัดสต๊อกเมื่อสร้าง
              </p>
            </div>
            <div className={`rounded-2xl border px-5 py-3 text-center ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-orange-50 border-orange-100'}`}>
              <p className={`text-2xl font-bold ${darkMode ? 'text-orange-400' : 'text-orange-700'}`}>{records.length}</p>
              <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-orange-600'}`}>ใบส่งสินค้าทั้งหมด</p>
            </div>
          </div>
