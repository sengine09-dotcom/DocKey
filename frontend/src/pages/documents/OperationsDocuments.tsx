      ...
      <div className={`min-h-screen ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
              <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>ระบบหลังบ้าน — ใบสั่งงาน</h1>
              <p className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                สร้างใบสั่งงาน → มอบหมาย → ติดตาม → ปิดงาน
              </p>
            </div>
            <div className={`rounded-2xl border px-5 py-3 text-center ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-rose-50 border-rose-100'}`}>
              <p className={`text-2xl font-bold ${darkMode ? 'text-rose-400' : 'text-rose-700'}`}>{records.length}</p>
              <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-rose-600'}`}>ใบสั่งงานทั้งหมด</p>
            </div>
          </div>
                      ...
                      <tr key={getRecordKey(record)} className={`transition-colors ${darkMode ? 'hover:bg-gray-700/40' : 'hover:bg-gray-50'}`}>
                        <td className="px-5 py-3.5">
                          <button type="button" onClick={() => handleView(record)}
                            className={`font-semibold hover:underline ${darkMode ? 'text-rose-400' : 'text-rose-600'}`}>
                        ...
                        <td className="px-5 py-3.5">
                          ...
                          <p className={`text-xs mt-0.5 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                            {record.assignedTo ? `👤 ${record.assignedTo}` : 'ยังไม่มอบหมาย'}
                          </p>
                        </td>
          ...
          {selectedRecord && !editorState && (
            <div className={`mt-6 rounded-2xl border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} shadow-sm p-6`}>
              <div className="flex items-center justify-between mb-4">
                <h2 className={`text-lg font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>🛠️ {selectedRecord.documentNumber}</h2>
