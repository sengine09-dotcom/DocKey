...
export default function VendorPickerModal({ isOpen, vendors, selectedCode, onSelect, onClear, onClose, darkMode }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return vendors.filter((v) =>
      [v.name, v.note, v.contactName, v.vendorCode]
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative w-full max-w-2xl rounded-2xl border shadow-2xl flex flex-col max-h-[80vh] ${bg}`}>
        ...
        <div className={`flex items-center justify-between gap-4 px-6 py-4 border-b rounded-t-2xl ${headerBg}`}>
          <div>
            <p className={`text-xs font-semibold uppercase tracking-wide ${textMuted}`}>เลือก Vendor</p>
            <h2 className={`mt-0.5 text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
              รายการ Vendor ทั้งหมด
            </h2>
          </div>
          <button
            type="button"
        ...
        <div className={`px-6 py-3 border-b ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
          <input
            ...
            placeholder="ค้นหาชื่อ, สินค้าที่ขาย, ผู้ติดต่อ..."
            className={inputCls}
          />
        </div>
        ...
        <div
          ...
          style={{ gridTemplateColumns: '90px 1fr 1fr 130px' }}
        >
          <div>Code</div>
          <div>ชื่อ Vendor</div>
          <div>Note / สินค้าที่ขาย</div>
          <div>ผู้ติดต่อ</div>
        </div>
        ...
        <div className="overflow-y-auto flex-1">
          {filtered.length === 0 ? (
            <div className={`px-6 py-12 text-center text-sm ${textMuted}`}>
              ไม่พบ Vendor ที่ค้นหา
            </div>
          ) : (
            filtered.map((v) => {
              const isSelected = v.vendorCode === selectedCode;
              return (
                <button
                  key={v.vendorCode}
                  type="button"
                  onClick={() => { onSelect(v); onClose(); setSearch(''); }}
                ...
                >
                  ...
                  <span className={`truncate ${textMuted}`} title={v.note || ''}>
                    {v.note || <span className="opacity-40">-</span>}
                  </span>
        ...
        <div className={`flex items-center justify-between px-6 py-3 border-t rounded-b-2xl ${headerBg}`}>
          <span className={`text-xs ${textMuted}`}>
            {filtered.length} vendor{filtered.length !== vendors.length ? ` (จาก ${vendors.length})` : ''}
          </span>
          <div className="flex gap-2">
            {selectedCode && (
              <button
                type="button"
                onClick={() => { onClear(); onClose(); setSearch(''); }}
